...
def process_next(db: Database, config: AppConfig) -> dict[str, Any]:
    ...
    if not path.exists():
        ...
        return {"ok": False, "status": "failed", "job_id": str(job["_id"]), **details}
    ...
    try:
        ...
        return {"ok": False, "status": "rejected", "job_id": str(job["_id"]), **details}
    ...
    return {"ok": True, "status": "completed", "job_id": str(job["_id"]), **inserted}
