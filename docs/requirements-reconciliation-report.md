...
| Active document registry | Session registry of referenced/produced/ingested docs (`REQ-ADR-01` to `REQ-ADR-03`) | Session registry exists for documents referenced by used nodes and output-ingested documents, and direct retrieval uses it for prompts such as "this document" | Still not a broad retrieval/fallback/endorsement driver | Connect registry to source fallback, endorsement workflows, and broader retrieval controls |
