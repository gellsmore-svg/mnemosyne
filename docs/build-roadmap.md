...
- identity, governance policy, process-object, process-run, and trust/temporal weighting schemas are planned in `docs/governance-schema-plan.md`; first read-only CLI/API listing and exact lookup commands, default seed records, memory-agent identity prompt summaries, agentic search identity exclusions, answer-process run persistence, and trust/temporal diagnostics in retrieval traces exist, but trust/temporal ranking effects and automatic process enforcement are not implemented yet. Remaining hardening includes process-run cleanup for exceptions before the adapter call and batching trust diagnostics if search limits grow.
