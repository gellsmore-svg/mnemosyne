...
- identity, governance policy, process-object, process-run, and trust/temporal weighting schemas are planned in `docs/governance-schema-plan.md`; first read-only CLI/API listing and exact lookup commands, default seed records, memory-agent identity prompt summaries, agentic search identity exclusions, and explicit process-run persistence exist, but trust/temporal weighting and automatic process enforcement are not implemented yet.
