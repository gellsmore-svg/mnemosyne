...
- FastAPI and the web operator panel can list supported files currently present in the configured ingest inbox; this is intentionally scoped to the ingest directory rather than exposing a general filesystem browser.
...
- The web UI separates Ask, Browse, and Ingestion into top-level tabs so operational ingest controls do not compete with the question-answering workflow. The Ask tab places Prompt, Response, and Activity Log in side-by-side panels on desktop, with Prompt / Trace relegated below the primary row and the JSON report collapsed inside the Activity Log panel. This preserves transparency while reducing scroll-heavy inspection.
- The Ask activity panel now starts with live client-side running milestones, then renders the returned process trace as an operator timeline. Python-to-LLM handoffs expose expandable human-readable prompt/context payloads, while full server-pushed per-step streaming remains deferred.
- The web UI now includes an e-paper display mode for Dasung-style monitors. It can be toggled from the top bar or opened with `?display=epaper`, uses high-contrast controls, avoids dark trace panels, reduces text scale compared with the first e-paper pass, and keeps Ask panels side by side with proportional columns where viewport width allows.
...
- Current automated suite at last full run: `297 passed`.
...
- Lifecycle next-phase requirements are captured in `docs/lifecycle-next-phase-requirements.md`; they prioritize repository rebuildability, quality-over-speed ingestion, human-readable LLM activity reports, internet temporary context/candidate knowledge boundaries, chronological RS/AMS corpus processing, and product self-awareness.
...
- Answer flows now expose a first `activity_report` structure for human-readable transparency across query understanding, context construction, response generation, LLM activity, and system functions; this is the initial implementation of the Lifecycle transparency-reporting requirement.
