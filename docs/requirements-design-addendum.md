...
- The web UI separates Ask, Browse, and Ingestion into top-level tabs so operational ingest controls do not compete with the question-answering workflow. The Ask tab places Prompt, Response, Activity Log, and Prompt / Trace in side-by-side panels on desktop, with the JSON report collapsed inside the Activity Log panel. This preserves transparency while reducing scroll-heavy inspection.
