...
- The web UI now includes an e-paper display mode for Dasung-style monitors. It can be toggled from the top bar or opened with `?display=epaper`, uses larger high-contrast controls, avoids dark trace panels, removes nested answer/log scrolling, and changes Ask into a single-column reading flow.
