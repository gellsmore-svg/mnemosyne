...
| Active document references support follow-up prompts such as "this document" in the same session. | Partial | Active document persistence, vocabulary, fallback, and tests exist. | Add or confirm a focused end-to-end session smoke for a reference-shaped prompt. |
...
| Full automated tests pass. | Done | Latest run: `473 passed`. | Re-run before release tag. |

## V1 Punch List

1. Add an explicit end-to-end test or smoke script for active-document follow-up prompts.
2. Audit README against the V1 CLI/web inspection surface.
3. Create a small release-candidate smoke corpus and document the exact smoke sequence.
4. Run the release-candidate smoke sequence through CLI and web.
5. Re-run the full test suite.
6. Tag V1 only after the checklist is complete and the working tree is clean.
