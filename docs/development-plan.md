...
- `mnemosyne enqueue-vector-semantic-batch` can queue profile-derived candidates across a bounded focus-node scope. This exists to make larger profile baselines reviewable without changing the review rule: candidates still require explicit acceptance before they become semantic relationships.
