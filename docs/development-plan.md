...
- `mnemosyne enqueue-vector-semantic-batch` can queue profile-derived candidates across a bounded focus-node scope. Repeated `--exclude-node-key` filters let operators skip templated focus rows such as concept-visit `section-1` headers. This exists to make larger profile baselines reviewable without changing the review rule: candidates still require explicit acceptance before they become semantic relationships.
