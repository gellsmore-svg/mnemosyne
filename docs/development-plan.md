...
- A real local `ollama_http` embedding adapter is available behind the same factory. It calls the local Ollama `/api/embed` endpoint using `runtime.embedding_model`, `runtime.ollama_base_url`, and `runtime.ollama_timeout_seconds`, validates that a vector was returned, normalizes it, and records the model and returned dimensions in each stored node embedding. The default remains `mock`.
