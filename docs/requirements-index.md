...
| Graph edges | REQ-ING-04, REQ-ING-05, REQ-ING-11 | First `graph_edges` collection persists ingestion relation hints when they reference known node keys; `get_graph_edges` exposes bounded single-hop lookup. Scored traversal and relation inference remain deferred. |
