...
- Primary retrieval is still lexical and locally reranked; vector/profile signals exist but are not yet a full hybrid lexical+vector retrieval path. A deterministic hybrid ranker (`hybrid_rank` in `retrieval/queries.py`, ADR-020) is wired into the agentic `search_nodes` tool, opt-in via `runtime.hybrid_search_enabled` and active only with a real (non-mock) embedding adapter; it is not yet the default and direct mode is not yet wired.
